...
void Simplex::MyCamera::CalculationHelper(float a_x, float a_y) //helper function to capture angles from cameraRotation
{
	m_x -= a_x; //get the x and y angles from rotateCamera
	m_y -= a_y;
}
...
void Simplex::MyCamera::CalculateViewMatrix(void)
{
	...
	quaternion orientation; //create quaternion
	quaternion pitch = glm::angleAxis(-m_x, AXIS_X); //get the pitch quaternion, invert x to make the axis align properly
	quaternion yaw = glm::angleAxis(m_y, AXIS_Y);  //get the yaw qauternion
	orientation = pitch * yaw;  //get the combined orientation

	matrix4 tempMat = glm::toMat4(orientation); //convert that quaternion to a matrix 4
	m_m4View = tempMat; //set the view matrix to the quaternion matrix to get the correct roations
	m_m4View = glm::translate(m_m4View, -m_v3Position); //translate the matrix to be a start position
...
{
	matrix4 m_m4ViewInverted = glm::inverse(m_m4View); //invert the matrix
	vector3 cameraLookVector = glm::normalize(glm::vec3(m_m4ViewInverted[2])); //get the normalized direction vector from the matrix
	//float dotVectorUp = glm::dot(cameraLookVector, m_v3Position); 
	m_v3Position += cameraLookVector * a_fDistance; //move the camera along the look vector 
	m_v3Target += cameraLookVector * a_fDistance; //adjust target
	m_v3Above += cameraLookVector * a_fDistance; //adjust above vector
}
